...
void change_double()
{
    struct student *temp = NULL;                                   /*定时临时变量用来存放头节点的地址*/
     
    for (temp = new_head; temp->next != NULL; temp = temp->next)   /*循环实现将每一个节点变成双向的*/
    {
        temp->next->pre = temp;
	debug_msg("age:%d\n",temp->next->pre->age);
    }
...
返回值  ：   无
***************************************************************************/
void search_student(int age)
{
    struct student *temp = NULL;
    for (temp = new_head; temp != NULL; temp = temp->next)
    {
        if ((temp->age == age) && (temp->pre == NULL))    /*如果是第一个学生则只打印他下面的学生*/
	{
	    printf("The first student's next name = %s; age = %d; sex = %d\n",temp->next->name, temp->next->age,temp->next->sex);
	    return;
	}
	
	if ((temp->next == NULL) && (temp->age == age))   /*如果是最后一个学生则打印他前面的那个学生*/
	{
	    printf("The last student's pre name = %s; age = %d; sex = %d\n",temp->pre->name, temp->pre->age,temp->pre->sex);
            return;
	}
	
	if(temp->age == age)                              /*如果是中间的学生则打印他前面和后面的学生*/
	{   
	    printf("The  student's next name = %s; age = %d; sex = %d\n",temp->next->name, temp->next->age,temp->next->sex);
	    printf("The  student's pre name = %s; age = %d; sex = %d\n",temp->pre->name, temp->pre->age,temp->pre->sex);
            return;
	}
    }
}
